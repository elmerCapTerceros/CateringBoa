...
import {
    ErpNavigationItem,
    ErpVerticalNavigationAppearance,
    ErpVerticalNavigationMode,
    ErpVerticalNavigationPosition,
} from '@erp/components/navigation/navigation.types';
import { ErpVerticalNavigationAsideItemComponent } from '@erp/components/navigation/vertical/components/aside/aside.component';
...
import { ErpVerticalNavigationGroupItemComponent } from '@erp/components/navigation/vertical/components/group/group.component';
...
import { ErpScrollbarDirective } from '@erp/directives/scrollbar/scrollbar.directive';
...
@Component({
    selector: 'erp-vertical-navigation',
    templateUrl: './vertical.component.html',
    styleUrls: ['./vertical.component.scss'],
    animations: erpAnimations,
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs: 'erpVerticalNavigation',
    imports: [
        ErpScrollbarDirective,
        ErpVerticalNavigationAsideItemComponent,
        ...
        ErpVerticalNavigationGroupItemComponent,
        ErpVerticalNavigationSpacerItemComponent,
    ]
})
export class ErpVerticalNavigationComponent
    implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    @Input() appearance: ErpVerticalNavigationAppearance = 'default';
    @Input() autoCollapse: boolean = true;
    @Input() inner: boolean = false;
    @Input() mode: ErpVerticalNavigationMode = 'side';
    @Input() name: string = this._erpUtilsService.randomId();
    @Input() navigation: ErpNavigationItem[];
    @Input() opened: boolean = true;
    @Input() position: ErpVerticalNavigationPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output()
    readonly appearanceChanged: EventEmitter<ErpVerticalNavigationAppearance> =
        new EventEmitter<ErpVerticalNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<ErpVerticalNavigationMode> =
        new EventEmitter<ErpVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> =
        new EventEmitter<boolean>();
    @Output()
    readonly positionChanged: EventEmitter<ErpVerticalNavigationPosition> =
        new EventEmitter<ErpVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<ErpNavigationItem> =
        new ReplaySubject<ErpNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<ErpNavigationItem> =
        new ReplaySubject<ErpNavigationItem>(1);
    ...
    private _erpScrollbarDirectives!: QueryList<ErpScrollbarDirective>;
    private _erpScrollbarDirectivesSubscription: Subscription;
    ...
    @HostBinding('class') get classList(): any {
        /* eslint-disable @typescript-eslint/naming-convention */
        return {
            'erp-vertical-navigation-animations-enabled':
                this._animationsEnabled,
            [`erp-vertical-navigation-appearance-${this.appearance}`]: true,
            'erp-vertical-navigation-hover': this._hovered,
            'erp-vertical-navigation-inner': this.inner,
            'erp-vertical-navigation-mode-over': this.mode === 'over',
            'erp-vertical-navigation-mode-side': this.mode === 'side',
            'erp-vertical-navigation-opened': this.opened,
            'erp-vertical-navigation-position-left': this.position === 'left',
            'erp-vertical-navigation-position-right':
                this.position === 'right',
        };
        /* eslint-enable @typescript-eslint/naming-convention */
    }
    ...
    /**
     * Setter for erpScrollbarDirectives
     */
    @ViewChildren(ErpScrollbarDirective)
    set erpScrollbarDirectives(
        erpScrollbarDirectives: QueryList<ErpScrollbarDirective>
    ) {
        // Store the directives
        this._erpScrollbarDirectives = erpScrollbarDirectives;

        // Return if there are no directives
        if (erpScrollbarDirectives.length === 0) {
            return;
        }

        // Unsubscribe the previous subscriptions
        if (this._erpScrollbarDirectivesSubscription) {
            this._erpScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._erpScrollbarDirectivesSubscription = merge(
            ...
            .subscribe(() => {
                ...
                erpScrollbarDirectives.forEach((erpScrollbarDirective) => {
                    erpScrollbarDirective.update();
                });
            });
    }
    ...
    ngAfterViewInit(): void {
        ...
        setTimeout(() => {
            ...
            if (
                ...
                const activeItem =
                    this._navigationContentEl.nativeElement.querySelector(
                        '.erp-vertical-navigation-item-active'
                    );
            ...
            // Otherwise
                ...
                this._erpScrollbarDirectives.forEach(
                    (erpScrollbarDirective) => {
                        // Skip if not enabled
                        if (!erpScrollbarDirective.isEnabled()) {
                            return;
                        }

                        // Scroll to the active element
                        erpScrollbarDirective.scrollToElement(
                            '.erp-vertical-navigation-item-active',
                            -120,
                            true
                        );
     ...
     * @param item
     */
    openAside(item: ErpNavigationItem): void {
     ...
     * @param item
     */
    toggleAside(item: ErpNavigationItem): void {
        // Toggle
    ...
    private _showOverlay(): void {
        ...
        this._overlay.classList.add('erp-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if (this.transparentOverlay) {
            this._overlay.classList.add(
                'erp-vertical-navigation-overlay-transparent'
            );
        }
    ...
    private _showAsideOverlay(): void {
        ...
        this._asideOverlay.classList.add(
            'erp-vertical-navigation-aside-overlay'
        );
