...
import {
    ScrollbarGeometry,
    ScrollbarPosition,
} from '@erp/directives/scrollbar/scrollbar.types';
 ...
 */
@Directive({
    selector: '[erpScrollbar]',
    exportAs: 'erpScrollbar',
    standalone: true,
})
export class ErpScrollbarDirective implements OnChanges, OnInit, OnDestroy {
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_erpScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    private _elementRef = inject(ElementRef);
    private _platform = inject(Platform);

    @Input() erpScrollbar: boolean = true;
    @Input() erpScrollbarOptions: PerfectScrollbar.Options;
    ...
    ngOnChanges(changes: SimpleChanges): void {
        // Enabled
        if ('erpScrollbar' in changes) {
            // Interpret empty string as 'true'
            this.erpScrollbar = coerceBooleanProperty(
                changes.erpScrollbar.currentValue
            );

            // If enabled, init the directive
            if (this.erpScrollbar) {
                this._init();
            }
        ...
        // Scrollbar options
        if ('erpScrollbarOptions' in changes) {
            // Merge the options
            this._options = merge(
                {},
                this._options,
                changes.erpScrollbarOptions.currentValue
            );
