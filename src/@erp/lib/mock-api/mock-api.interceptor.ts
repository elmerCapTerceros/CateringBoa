...
import { ERP_MOCK_API_DEFAULT_DELAY } from '@erp/lib/mock-api/mock-api.constants';
import { ErpMockApiService } from '@erp/lib/mock-api/mock-api.service';
...
export const mockApiInterceptor = (
    ...
    const defaultDelay = inject(ERP_MOCK_API_DEFAULT_DELAY);
    const erpMockApiService = inject(ErpMockApiService);

    // Try to get the request handler
    const { handler, urlParams } = erpMockApiService.findHandler(
